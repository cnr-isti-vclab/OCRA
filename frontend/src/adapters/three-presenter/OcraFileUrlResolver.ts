...
function rewriteAbsoluteAssetsUrlIfNeeded(filePath: string, apiBase: string): string {
  ...
  try {
    ...
    // Scene payloads may contain absolute assets URLs generated by a different
    // host/port (for example localhost or host without :3001). Re-anchor those
    // URLs to the currently configured API base.
    if (parsed.pathname.startsWith("/assets/projects/")) {
      const base = normalizeBaseUrl(apiBase);
      return `${base}${parsed.pathname}${parsed.search}${parsed.hash}`;
    }
  } catch {
    // If URL parsing fails, keep the original behavior and pass through.
  }
...
export class OcraFileUrlResolver implements FileUrlResolver {
  ...
  resolve(filePath: string, context: FileResolverContext): string {
    ...
    // Absolute URLs are usually passed through, but OCRA assets URLs are
    // normalized to the configured API base to avoid host/port drift.
    if (filePath.startsWith("http://") || filePath.startsWith("https://")) {
      return rewriteAbsoluteAssetsUrlIfNeeded(filePath, apiBase);
    }
